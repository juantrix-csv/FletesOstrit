...
export default function AdminJobs() {
  ...
  const driverVehicleDriverShareRatio = driverVehicleDriverSharePercentValue != null
    ...
    : (2 / 3);
  const getVehicleCompanyHourlyMargin = (vehicle: Vehicle | null) => (
    Number.isFinite(vehicle?.companyHourlyMargin)
  ...
  const hasVehicleHourlyRates = useMemo(
    () => vehicles.some((vehicle) => Number.isFinite(vehicle.hourlyRate)),
    [vehicles],
  );
  const getDriverDefaultVehicleId = (driverId?: string | null) => {
    if (!driverId) return '';
    return driversById.get(driverId)?.vehicleId ?? '';
  };
  const getJobVehicle = (job: Pick<Job, 'vehicleId' | 'driverId'>) => {
    if (job.vehicleId) return vehiclesById.get(job.vehicleId) ?? null;
    const driverVehicleId = getDriverDefaultVehicleId(job.driverId);
    return driverVehicleId ? vehiclesById.get(driverVehicleId) ?? null : null;
  };
  const getJobHourlyRateValue = (job: Pick<Job, 'vehicleId' | 'driverId' | 'hourlyRateSnapshot'>) => {
    if (Number.isFinite(job.hourlyRateSnapshot)) return Number(job.hourlyRateSnapshot);
  ...
  const getJobLongDistanceCalculatedTotal = (job: Job) => {
    ...
    const snapshotPrice = Number.isFinite(job.pricePerLongDistanceKmSnapshot)
      ? Number(job.pricePerLongDistanceKmSnapshot)
      : getJobLongDistancePricePerKmValue(job);
    return distanceKm != null && snapshotPrice != null && snapshotPrice > 0
      ? roundMoney(distanceKm * snapshotPrice)
      : null;
  };
  ...
  const handleAssignJob = async (job: Job, driverId: string) => {
    const vehicleId = driverId ? getDriverDefaultVehicleId(driverId) || null : null;
    try {
      const updated = await updateJob(job.id, { driverId: driverId || null, vehicleId });
  ...
  const handleAssignJobVehicle = async (job: Job, vehicleId: string) => {
    try {
      const updated = await updateJob(job.id, { vehicleId: vehicleId || null });
      setJobs((prev) => prev.map((item) => (item.id === job.id ? updated : item)));
  ...
  const handleSaveVehicle = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const hourlyRate = parseMoneyInput(vehicleHourlyRateInput);
    if (vehicleHourlyRateInput.trim() && hourlyRate == null) {
      toast.error('Precio por hora invalido');
      return;
    }
    const companyHourlyMargin = parseMoneyInput(vehicleCompanyHourlyMarginInput);
    if (vehicleCompanyHourlyMarginInput.trim() && companyHourlyMargin == null) {
  ...
  const getEntryHourlyValue = (entry: { job: Job; durationMs: number | null }) => {
    ...
    const jobHourlyRate = getJobHourlyRateValue(entry.job);
    if (jobHourlyRate != null && billedHours != null) {
      return billedHours * jobHourlyRate;
    }
  ...
  const getEntryTotal = (entry: { job: Job; durationMs: number | null }) => {
    ...
    const effectiveHelperRate = entry.job.helperHourlyRateSnapshot ?? helperHourlyRateValue;
  ...
  const getEntryVehicle = (entry: { job: Job }) => {
    return getJobVehicle(entry.job);
  };
  ...
  const getEntryExpenseBreakdown = (entry: { job: Job; durationMs: number | null }) => {
    ...
    const effectiveEntryHelperRate = entry.job.helperHourlyRateSnapshot ?? helperHourlyRateValue;
    const helpersCost = effectiveEntryHelperRate != null && paidHelpersCount > 0 && billedHours != null
      ? billedHours * effectiveEntryHelperRate * paidHelpersCount
      : 0;
  ...
  }, [completedHistory, hourlyRateValue, helperHourlyRateValue, vehiclesById, driversById]);
  ...
  const monthlyGrossTotal = useMemo(() => {
    if (hourlyRateValue == null && !hasVehicleHourlyRates && !hasChargeOverrides) return null;
    const now = new Date();
  ...
  const currentMonthCollectedLabel = currentMonthPaymentBreakdown != null
    ...
    : currencyFormatter.format(0);
  const hasMonthlyPricing = hourlyRateValue != null || hasVehicleHourlyRates || helperHourlyRateValue != null || hasChargeOverrides;
  ...
  const calendarEstimateSummary = useMemo(() => {
    ...
    scheduledJobs.forEach((item) => {
      ...
      const ownerPaysHelpers = isOwnerAccountDriver(driver);
      const paidHelpersCount = ownerPaysHelpers ? getPaidHelpersCountForOwner(helpersCount) : 0;
      const effectiveForecastHelperRate = item.job.helperHourlyRateSnapshot ?? helperHourlyRateValue;
      const helpersCost = effectiveForecastHelperRate != null && paidHelpersCount > 0 && billedHours != null
        ? billedHours * effectiveForecastHelperRate * paidHelpersCount
        : 0;
                    ...
                    <form onSubmit={addJob} className="space-y-4 rounded bg-white p-3 shadow sm:p-4">
                      ...
                      <div className="grid gap-3 sm:grid-cols-2">
                        <label className="text-xs text-gray-500">
                          ...
                          <select
                            name="driverId"
                            value={newJobDriverId}
                            onChange={(event) => {
                              const driverId = event.target.value;
                              setNewJobDriverId(driverId);
                              setNewJobVehicleId(getDriverDefaultVehicleId(driverId));
                            }}
                            className="mt-1 w-full rounded border px-3 py-2 text-sm"
                          >
                        ...
                        <label className="text-xs text-gray-500">
                          ...
                          <select
                            ...
                            onChange={(event) => setNewJobVehicleId(event.target.value)}
                            className="mt-1 w-full rounded border px-3 py-2 text-sm"
                          >
                            <option value="">Sin vehiculo especifico</option>
                            {vehicles.map((vehicle) => (
                              <option key={vehicle.id} value={vehicle.id}>
                                {vehicle.name}
                                {Number.isFinite(vehicle.hourlyRate) ? ` - ${currencyFormatter.format(Number(vehicle.hourlyRate))}/h` : ''}
                              </option>
                            ))}
                          </select>
                        </label>
                        ...
                        {pickup && dropoff && !loadingBaseTravelEstimate && baseTravelEstimate && (
                          <div className="mt-2 space-y-1 text-blue-900">
                            ...
                            <p>
                              ...
                              Si el punto mas lejano supera 15 min desde la base, ese tiempo se suma al cobro final.
                              </span>
                            </p>
                          </div>
                        )}
                        ...
                        {!isEditing && (
                          <div className="flex min-w-0 flex-wrap items-center gap-2">
                            ...
                            <select
                              value={job.vehicleId ?? ''}
                              onClick={(event) => event.stopPropagation()}
                              onChange={(event) => handleAssignJobVehicle(job, event.target.value)}
                              className="w-full max-w-full rounded border px-2 py-1 text-xs sm:w-auto"
                            >
                              <option value="">Predeterminado del conductor</option>
                              {vehicles.map((vehicle) => (
                                <option key={vehicle.id} value={vehicle.id}>
                                  {vehicle.name}
                                  {Number.isFinite(vehicle.hourlyRate) ? ` - ${currencyFormatter.format(Number(vehicle.hourlyRate))}/h` : ''}
                            ...
                            {jobVehicle && (
                              <span className="text-xs text-gray-500">
                                Usa: {jobVehicle.name}
                                {Number.isFinite(jobVehicle.hourlyRate) ? ` (${currencyFormatter.format(Number(jobVehicle.hourlyRate))}/h)` : ''}
                              </span>
                            )}
                          </div>
                        )}
                        {isEditing && (
                          <div className="rounded border bg-gray-50 p-3" onClick={(event) => event.stopPropagation()}>
                                ...
                                <select
                                  ...
                                  onChange={(event) => {
                                    ...
                                    setEditDraft((prev) => ({
                                      ...prev,
                                      driverId,
                                      vehicleId: getDriverDefaultVehicleId(driverId),
                                    }));
                                  }}
                                  className="mt-1 w-full rounded border px-2 py-1 text-xs text-gray-700"
                                >
                              ...
                              <label className="text-xs text-gray-500">
                                Vehiculo
                                <select
                                  value={editDraft.vehicleId}
                                  onChange={(event) => setEditDraft((prev) => ({ ...prev, vehicleId: event.target.value }))}
                                  className="mt-1 w-full rounded border px-2 py-1 text-xs text-gray-700"
                                >
                                  <option value="">Predeterminado del conductor</option>
                                  {vehicles.map((vehicle) => (
                      ...
                      >
                        ...
                        <div className="relative border-l border-gray-100" style={{ height: calendarGridHeight }}>
                          ...
                          {dayJobs.map((item) => {
                            ...
                            const vehicle = getJobVehicle(item.job);
                            const calendarOwnerLabel = vehicle ? `${driverLabel} | ${vehicle.name}` : driverLabel;
            ...
            <div className="space-y-4">
              ...
              <div className="rounded-2xl border bg-white p-4 shadow-sm space-y-3">
                ...
                <form onSubmit={handleSaveVehicle} className="grid gap-3 sm:grid-cols-2 lg:grid-cols-[2fr_1fr_1fr_1fr_1fr_1fr_1fr_1fr_auto]">
                  ...
                  <div>
                    <label className="text-xs text-gray-500">Precio hora</label>
                    <input
                      value={vehicleHourlyRateInput}
                      onChange={(event) => setVehicleHourlyRateInput(event.target.value)}
                  ...
                  <div>
                    <label className="text-xs text-gray-500">Margen dueno</label>
                    <input
                      value={vehicleCompanyHourlyMarginInput}
                      onChange={(event) => setVehicleCompanyHourlyMarginInput(event.target.value)}
                      placeholder="Ej: 10000"
                      type="number"
                      min="0"
                      step="0.01"
                      disabled={vehicleOwnershipType !== 'driver'}
                    ...
                    <div className="space-y-2">
                      {vehicles.map((vehicle) => (
                        <div key={vehicle.id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-gray-100 bg-gray-50 px-3 py-2 text-sm">
                          ...
                          <div className="text-xs text-gray-600">
                            <span className="font-semibold text-gray-800">
                              {Number.isFinite(vehicle.hourlyRate) ? currencyFormatter.format(Number(vehicle.hourlyRate)) : 'Usa precio global'}
                            </span> / h
                          </div>
                          {vehicle.ownershipType === 'driver' && (
                            <div className="text-xs text-gray-600">
                              <span className="font-semibold text-gray-800">{currencyFormatter.format(getVehicleCompanyHourlyMargin(vehicle))}</span> margen
                            </div>
                          )}
                          <div className="text-xs text-gray-600">
                ...
                {completedHistory.length === 0 ? (
                  ...
                  <div className="mt-3 space-y-2">
                    {completedHistory.map((entry) => {
                      ...
                      const vehicle = getJobVehicle(entry.job);
                      const hourlyDistribution = getEntryHourDistribution(entry);
                      ...
                      const effectiveHelperRate = entry.job.helperHourlyRateSnapshot ?? helperHourlyRateValue;
                      const helpersValue = effectiveHelperRate != null && billedHours != null && helpersCount > 0
                        ? billedHours * effectiveHelperRate * helpersCount
                        : null;
                          ...
                          <div className="flex flex-wrap items-start justify-between gap-3">
                            <div>
                              ...
                              <p className="text-sm text-gray-500">
                                Vehiculo: {vehicle
                                  ? `${vehicle.name} (${vehicleOwnershipLabels[vehicle.ownershipType]}${Number.isFinite(vehicle.hourlyRate) ? `, ${currencyFormatter.format(Number(vehicle.hourlyRate))}/h` : ''})`
              ...
              <div className="rounded-2xl border bg-white p-4 shadow-sm">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="text-xs text-gray-500">
                      Se usa para calcular la lejania y marcar la referencia sin extra.
                    </p>
                  </div>
                ...
                <div className="mt-4 grid gap-4 lg:grid-cols-[1.2fr_1fr]">
                  ...
                  <div className="rounded-xl border bg-gray-50 p-3 text-sm text-gray-600">
                    ...
                    <p className="mt-2 text-xs text-gray-500">
                      El area verde marca una referencia de 15 min desde la base. El cobro final usa el tiempo estimado al punto mas lejano.
                    </p>
                  </div>
                </div>
              ...
              <div className="grid gap-4 lg:grid-cols-[1.1fr_1fr]">
                <div className="rounded-2xl border bg-white p-4 shadow-sm">
                  ...
                  <div className="mt-4 border-t border-gray-100 pt-4">
                    ...
                    <p className="text-xs text-gray-500">
                      Vehiculo del dueno: {ownerVehicleDriverShareLabel} para chofer. El margen de vehiculo del chofer se define en cada vehiculo.
                    </p>
                    <div className="mt-3 grid gap-4 sm:grid-cols-2">
                      <div>
                      ...
                      <div>
                        ...
                        <p className="mt-2 rounded border border-cyan-100 bg-cyan-50 px-2 py-1 text-xs text-cyan-800">
                          Cargalo en la tarjeta Vehiculos, campo Margen dueno. Si queda vacio usa {driverOwnedVehicleCompanyMarginLabel}/h.
                        </p>
                      </div>
                    </div>
                  </div>
