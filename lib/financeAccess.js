...
const buildFinanceJob = (job, context) => {
  ...
  const vehicle = driver?.vehicleId ? context.vehiclesById.get(driver.vehicleId) ?? null : null;
  const startMs = getJobStartMs(job);
  const endMs = getJobEndMs(job);
  ...
  const hourlyBaseAmount = toFiniteNumberOrNull(job.hourlyBaseAmount)
    ?? (context.settings.hourlyRate != null && billedHours != null
      ? roundMoney(context.settings.hourlyRate * billedHours)
      : null);
