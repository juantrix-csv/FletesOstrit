...
export const FINANCE_SETTING_KEYS = [
  ...
  'driverVehicleCompanyHourlyMargin',
];
...
const normalizeSettings = (settings = {}) => ({
  ...
  driverVehicleCompanyHourlyMargin: getSettingsValue(settings, 'driverVehicleCompanyHourlyMargin'),
});
...
const buildFinanceJob = (job, context) => {
  ...
  const vehicle = job.vehicleId
    ...
    : driver?.vehicleId
      ...
      : null;
  const effectiveHourlyRate = toFiniteNumberOrNull(job.hourlyRateSnapshot)
    ?? toFiniteNumberOrNull(vehicle?.hourlyRate)
    ?? context.settings.hourlyRate;
  const startMs = getJobStartMs(job);
  const endMs = getJobEndMs(job);
  ...
  const hourlyBaseAmount = toFiniteNumberOrNull(job.hourlyBaseAmount)
    ?? (effectiveHourlyRate != null && billedHours != null
      ...
      : null);
  const effectiveHelperRate = toFiniteNumberOrNull(job.helperHourlyRateSnapshot)
    ?? context.settings.helperHourlyRate;
  const helpersAmount = effectiveHelperRate != null && billedHours != null && helpersCount > 0
    ? roundMoney(effectiveHelperRate * billedHours * helpersCount)
    : 0;
  ...
  return {
    ...
    vehicle: vehicle ? {
      ...
      companyHourlyMargin: toFiniteNumberOrNull(vehicle.companyHourlyMargin),
    } : null,
    helpersCount,
  ...
  financeJobs.forEach((job) => {
    ...
    if (job.driver?.id) {
      ...
      if (driverSummary) {
        ...
        const driverKeptAmount = isExternalDriver(driverSummary)
          ? Math.max(0, Math.min(job.totalBilled ?? 0, job.shares.driverShareAmount ?? 0))
          : (job.totalBilled ?? 0);
        const grossOwnerDebt = isExternalDriver(driverSummary)
          ? Math.max(0, (job.totalBilled ?? 0) - driverKeptAmount)
          : 0;
        driverSummary.grossOwnerDebt += grossOwnerDebt;
        driverSummary.driverKeptAmount += driverKeptAmount;
      }
    }
...
const normalizeVehicles = (vehicles) => toArray(vehicles).map((vehicle) => ({
  ...
  companyHourlyMargin: toFiniteNumberOrNull(vehicle.companyHourlyMargin),
  costPerKm: toFiniteNumberOrNull(vehicle.costPerKm) ?? 0,
  pricePerLongDistanceKm: toFiniteNumberOrNull(vehicle.pricePerLongDistanceKm),
  fixedMonthlyCost: toFiniteNumberOrNull(vehicle.fixedMonthlyCost) ?? 0,
